...
export default function SupplierOrderForm({ onSuccess }) {
  ...
  const submit = async (e) => {
    ...
    if (items.some(i => !i.productId || !i.quantity || !i.unitPrice)) {
      setError('Veuillez compléter tous les items')
      return
    }
    setLoading(true)
    try {
  ...
  return (
    <form onSubmit={submit} className="space-y-4">
      ...
      <h3 className="font-semibold text-gray-700">Articles commandés</h3>
      ...
      <div>
        ...
        <textarea value={notes} onChange={e => setNotes(e.target.value)} rows={2} className="input-field" placeholder="Référence devis, remarques..." />
      </div>

      <button type="submit" disabled={loading} className="btn-primary w-full">
        {loading ? 'Création...' : '🛒 Créer la commande fournisseur'}
      </button>
    </form>
  )
}
